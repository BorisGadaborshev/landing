...
const Card = styled.section`
  ...
  background: #ebe6df;
  border-radius: ${u(32)};
  box-shadow: ${u(3)} ${u(-4)} ${u(10)} rgba(0, 0, 0, 0.34);
  color: #0e1116;
  z-index: 2;
...
const PrimaryButton = styled.button<{ $left: number; $top: number }>`
  ...
  font-size: ${u(32)};
  line-height: 1.12;
  letter-spacing: ${u(0.35)};
  cursor: pointer;
  transition: transform 0.15s ease, box-shadow 0.15s ease;

  &:hover {
    transform: translateY(${u(-1)});
    box-shadow: 0 ${u(8)} ${u(20)} rgba(0, 0, 0, 0.24);
  }
`
  ...
  return (
    <>
      <Card>
        ...
        />
        <PrimaryButton type="button" $left={128} $top={1041} onClick={handleUploadClick}>
          Загрузить своё фото
        </PrimaryButton>
        <Hint $left={148} $top={1127} $width={472}>
          (или просто перетащите сюда ваше изображение)
        </Hint>
        <PrimaryButton type="button" $left={796} $top={1041} onClick={handleGenerateClick}>
          Сгенерировать
        </PrimaryButton>
